...
def part_1(save_plot=False):
    ...
    x_j, iterations_j, norms_j, time_j = jacobi(N, A, b, 1000, 10**(-9), save=False)
    print(f"Jacobi: iterations - {iterations_j}, time - {time_j}")
    
    x_gs, iterations_gs, norms_gs, time_gs = gauss_seidel(N, A, b, 1000, 10**(-9), save=False)
...
def part_4(save_plot=False):
    ...
    for n in n_tab:
        A = A_band(n, a1, a2, a3)
        b = B_sin(n)
        _, _, _, time_j = jacobi(n, A, b, 1000, 10**(-9), save=False)
        j_times.append(time_j)
        _, _, _, time_gs = gauss_seidel(n, A, b, 1000, 10**(-9), save=False)
        gs_times.append(time_gs)
        _, time_lu = lu_factorization(A, b, save=False)
